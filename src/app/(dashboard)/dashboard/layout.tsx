...
const Layout = async ({children}: LayoutProps) => {
    ...
    const unseenRequestCount = (
        await fetchRedis('smembers',
            `user:${session.user.id}:incoming_friend_requests`) as User[]
    ).length
    ...
    return (
        <div className='w-full flex h-screen'>
            ...
            <div
                className='hidden flex h-full w-full max-w-xs grow flex-col gap-y-5 overflow-y-auto border-r border-gray-200 bg-white px-6'>
                <Link href='/dashboard'
                ...
                <nav className='flex flex-1 flex-col'>
                    <ul role='list' className='flex flex-1 flex-col gap-y-7'>
                        ...
                        <li className='-mx-6 mt-auto items-center flex'>
                            <div
                                className='flex flex-1 items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-5 text-gray-900'>
